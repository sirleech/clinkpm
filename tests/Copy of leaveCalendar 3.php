...
for ($month = 1; $month <= 12; $month++) {
	...
	// a array of staff leave plans (a staff member can have 0..m leave plans that fall in one month)
	...
	while ($row = mysql_fetch_assoc($leavePlansInThisMonth)) {
		...
		$leavePlans[$row['user_name']] = $leavePlan;		
		$staffLeavePlans[$row['user_name']] = new StaffLeavePlanDto();
		
	}
	
	// consolidate leavePlans into staffLeavePlans
	foreach($leavePlans as $i => $value)
	{
		$tempArray = $staffLeavePlans[$i]->leavePlans;
		array_push($tempArray,$leavePlans[$i]);
		$staffLeavePlans[$i] = $tempArray;
	}
